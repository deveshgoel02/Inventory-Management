...
export default function AnalyticsPage() {
  ...
  return (
    <div>
      ...
      <div className="text-sm font-semibold text-slate-800 mb-2">Business Insights</div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-6">
        {insights.map((i) => (
          <div key={i.id} className="bg-white border border-slate-200 rounded-lg p-3">
            <div className="flex items-center gap-2 mb-1">
              <Badge label={i.severity} />
              <span className="text-[11px] text-slate-400 uppercase">{i.category.replace(/_/g, " ")}</span>
            </div>
            <div className="text-sm text-slate-800">{i.text}</div>
          </div>
        ))}
        {insights.length === 0 && <div className="text-xs text-slate-400">No notable patterns detected yet.</div>}
      </div>

      <div className="flex items-center justify-between mb-2">
        <div className="text-sm font-semibold text-slate-800">Movement Classification</div>
        <div className="flex gap-1">
          <button onClick={() => setFilter("")} className={`text-xs px-2 py-1 rounded border ${filter === "" ? "border-indigo-500 bg-indigo-50" : "border-slate-200"}`}>
            All
          </button>
          {classes.map((c) => (
            <button key={c} onClick={() => setFilter(c)} className={`text-xs px-2 py-1 rounded border ${filter === c ? "border-indigo-500 bg-indigo-50" : "border-slate-200"}`}>
              {c.replace(/_/g, " ")}
            </button>
          ))}
        </div>
      ...
      <div className="bg-white rounded-lg border border-slate-200 overflow-x-auto">
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((c) => (
              <tr key={c.variant_id} className="border-b border-slate-50 hover:bg-slate-50">
                ...
                <td className="py-2 px-3">
                  {c.product_name} <span className="text-slate-400">({c.brand_name})</span>
                </td>
                <td className="py-2 px-3">
                ...
                <td className="py-2 px-3 text-xs text-slate-500 max-w-xs">{c.explanation}</td>
              </tr>
            ))}
          </tbody>
        </table>
